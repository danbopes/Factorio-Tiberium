	...
	for labName, labData in pairs(data.raw.lab) do
		if flib_table.find(labData.inputs or {}, "tiberium-science") and (labName ~= "creative-mod_creative-lab") then
			for _, pack in pairs(labData.inputs or {}) do
				if (pack ~= "tiberium-science") and ((data.raw.tool and data.raw.tool[pack]) or data.raw.item[pack]) then
	...
	for recipe in pairs(availableRecipes) do
		...
		if not next(resultList) then
			...
			if data.raw.recipe[recipe] and not fakeRecipes[recipe] then
				local recipeCategories = data.raw.recipe[recipe].categories
				local category = recipeCategories and recipeCategories[1]
